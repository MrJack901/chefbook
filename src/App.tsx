  ...
  return (
    <div>
      {steps.map((step, i) => (
        <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 10, alignItems: 'flex-start' }}>
          <div style={{
            ...
            marginTop: 8, flexShrink: 0, fontFamily: "'Cormorant Garamond',serif"
          }}>{i + 1}</div>
          <textarea
            ref={el => { refs.current[i] = el; if (el) autoResize(el); }}
            value={step}
            placeholder={i === 0 ? "Descrivi il primo passo... (Invio = nuovo passo, Shift+Invio = a capo)" : "Passo successivo..."}
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) => { update(i, e.target.value); autoResize(e.target); }}
            onKeyDown={(e) => handleKeyDown(e, i)}
            rows={1}
            style={{ ...inputStyle, flex: 1, minHeight: 44, lineHeight: 1.8, resize: 'none', overflow: 'hidden' }}
          />
          {steps.length > 1 && (
            <button
              onClick={() => { const s = steps.filter((_, j) => j !== i); onChange(s.length ? s : ['']); }}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: c.muted, fontSize: 18, marginTop: 4, padding: '0 4px', lineHeight: '1' }}
            >×</button>
          )}
        </div>
      ))}
      <button
        ...
        style={{ background: 'none', border: `1.5px dashed ${c.border}`, borderRadius: 8, padding: '7px 16px', color: c.muted, fontSize: 13, cursor: 'pointer', marginTop: 4, fontFamily: "'Nunito',sans-serif", fontWeight: 600 }}
    ...
    <div style={{ position: 'relative' }}>
      ...
      {open && filtered.length > 0 && (
        <div style={{
          ...
          background: '#fff', border: '1.5px solid #F2E4D0', borderTop: 'none',
          borderRadius: '0 0 8px 8px', boxShadow: '0 8px 20px rgba(100,70,30,0.08)'
        }}>
          {filtered.map(s => (
            <div key={s}
              onMouseDown={() => { onChange(s); setOpen(false); }}
              style={{
                padding: '10px 14px', cursor: 'pointer', fontSize: 13, color: '#8A7A65',
                textTransform: field === 'type' ? 'capitalize' : 'none', transition: 'background .15s'
              }}
...
export default function ChefBook() {
  ...
  const openDetail = (r: Recipe) => {
    setCurrent(r); setCurrentServings(r.servings || 1); setView('detail');
  };
  ...
  const handlePhotoSelect = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      sf('photo_url', url); setPhotoPreview(url);
  ...
  const A: Record<string, CSSProperties> = {
    ...
    header: { background: c.card, borderBottom: `1px solid ${c.border}`, padding: '0 24px', height: 62, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, gap: 12, boxShadow: `0 2px 12px ${c.shadow}` },
    logo: { fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 700, color: c.accent, letterSpacing: '.03em', whiteSpace: 'nowrap' },
    btn: { background: c.accent, color: '#FFF8F0', border: 'none', borderRadius: 8, padding: '9px 20px', fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', boxShadow: `0 2px 8px ${c.accent}40` },
    btnO: { background: 'transparent', color: c.muted, border: `1.5px solid ${c.border}`, borderRadius: 8, padding: '8px 16px', fontFamily: "'Nunito',sans-serif", fontWeight: 600, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap' },
    btnRed: { background: c.redLight, color: c.red, border: `1.5px solid #F5C6C2`, borderRadius: 8, padding: '8px 16px', fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 13, cursor: 'pointer' },
    input: { background: c.input, color: c.text, border: `1.5px solid ${c.border}`, borderRadius: 8, padding: '10px 14px', fontSize: 14, width: '100%', fontFamily: "'Nunito',sans-serif" },
    label: { fontSize: 11, fontWeight: 700, color: c.muted, letterSpacing: '.1em', textTransform: 'uppercase', display: 'block', marginBottom: 6 },
    field: { marginBottom: 18 },
    secTitle: { fontFamily: "'Cormorant Garamond',serif", fontSize: 21, fontWeight: 700, color: c.accent, marginBottom: 16, paddingBottom: 10, borderBottom: `2px solid ${c.accentLight}` },
    tag: { background: c.accentLight, color: c.accent, borderRadius: 20, padding: '3px 12px', fontSize: 11, fontWeight: 700, letterSpacing: '.05em', textTransform: 'capitalize' },
  ...
  if (view === 'loading') return (
    <div style={{ ...A.wrap, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ color: c.muted, fontSize: 13 }}><span className="spin">⟳</span> Connessione a Supabase...</div>
    </div>
  );

  // ─── LOGIN ─────────────────────────────────────────────────────────
  if (view === 'login') return (
    <div style={{ ...A.wrap, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'linear-gradient(135deg, #F7F3EE 0%, #EDE3D6 100%)' }}>
      <style>{css}</style>
      <div style={{ ...A.cardBox, padding: '52px 44px', maxWidth: 420, width: '100%', textAlign: 'center' }}>
        <div style={{ fontSize: 56, marginBottom: 16 }}>👨‍🍳</div>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 46, fontWeight: 700, color: c.accent, marginBottom: 6 }}>Chef's Book</div>
        <div style={{ color: c.muted, marginBottom: 36, fontSize: 14, lineHeight: 1.7 }}>Il ricettario collaborativo<br />della tua cucina</div>
        ...
        <div style={{ color: c.muted, fontSize: 11, marginTop: 20 }}>Ricette condivise con tutti i colleghi · Dati su Supabase</div>
      </div>
    </div>
  );
    ...
    <div style={A.wrap}>
      ...
      <div style={A.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, justifyContent: 'flex-end' }}>
          <input style={{ ...A.input, maxWidth: 190, padding: '7px 12px', fontSize: 13 }} placeholder="🔍 Cerca ricetta..." value={search} onChange={(e: ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} />
          <button className="hbtn" style={{ ...A.btnO, padding: '7px 12px', fontSize: 17 }} onClick={() => load()}>
      ...
      <div style={{ background: c.card, borderBottom: `1px solid ${c.border}`, padding: '0 24px', display: 'flex', gap: 4, flexWrap: 'wrap' }}>
        {['tutti', ...types].map(t => (
          <button key={t} onClick={() => setFilter(t)} style={{
            ...
            padding: '14px 18px 12px', fontSize: 13, cursor: 'pointer',
      ...
      <div style={{ padding: '12px 24px 4px', color: c.muted, fontSize: 12, fontWeight: 600 }}>
        {filtered.length} ricett{filtered.length === 1 ? 'a' : 'e'}{filter !== 'tutti' ? ` · ${filter}` : ''}
      </div>
      {error && <div style={{ ...A.err, margin: '4px 24px 8px' }}>{error}</div>}

      {filtered.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 24px', gap: 14, textAlign: 'center' }}>
          <div style={{ fontSize: 60 }}>📖</div>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 28, color: c.muted }}>
            {search ? 'Nessun risultato' : 'Nessuna ricetta ancora'}
          </div>
          <div style={{ color: c.muted, fontSize: 14 }}>{search ? `Nessuna ricetta per "${search}"` : 'Aggiungi la prima ricetta al vostro libro!'}</div>
          {!search && <button className="hbtn" style={{ ...A.btn, marginTop: 8 }} onClick={newRecipe}>+ Aggiungi ricetta</button>}
        </div>
      ) : (
        <div className="rgrid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 18, padding: 24 }}>
          {filtered.map(r => (
            <div key={r.id} className="hcard" style={{ background: c.card, borderRadius: 14, border: `1px solid ${c.border}`, overflow: 'hidden', cursor: 'pointer', boxShadow: `0 2px 10px ${c.shadow}` }}
              ...
              {r.photo_url ? (
                <img src={r.photo_url} alt={r.title} style={{ width: '100%', height: 155, objectFit: 'cover', display: 'block' }} />
              ) : (
                <div style={{ width: '100%', height: 155, background: `linear-gradient(135deg, ${c.accentLight} 0%, #EDD5B0 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontSize: 46 }}>🍽️</span>
                </div>
              )}
              <div style={{ padding: '13px 16px 15px' }}>
                {r.type && <div style={{ ...A.tag, display: 'inline-block', marginBottom: 8 }}>{r.type}</div>}
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 700, lineHeight: 1.3, marginBottom: 8 }}>{r.title}</div>
                ...
                <div style={{ color: c.muted, fontSize: 11, marginTop: 8, borderTop: `1px solid ${c.border}`, paddingTop: 8 }}>
      ...
      <div style={A.wrap}>
        ...
        <div style={A.header}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="hbtn" style={A.btnRed} onClick={() => { if (window.confirm(`Eliminare "${r.title}"?`)) handleDelete(r.id); }}>🗑 Elimina</button>
          </div>
        </div>

        <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 24px 70px' }}>
          {r.photo_url && (
            <img src={r.photo_url} alt={r.title} style={{ width: '100%', height: 320, objectFit: 'cover', borderRadius: 16, marginBottom: 28, display: 'block', boxShadow: `0 8px 32px ${c.shadow}` }} />
          )}
          {r.type && <div style={{ ...A.tag, display: 'inline-block', marginBottom: 12 }}>{r.type}</div>}
          <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 42, fontWeight: 700, lineHeight: 1.15, marginBottom: 24, color: c.text }}>{r.title}</h1>

          <div style={{ display: 'flex', flexWrap: 'wrap', marginBottom: 28, background: c.accentLight, borderRadius: 12, overflow: 'hidden', border: `1px solid ${c.border}` }}>
            {([
              r.creation_time ? { icon: '⏱', label: 'Tempo', value: r.creation_time } : null,
              r.weight ? { icon: '⚖️', label: 'Peso', value: r.weight } : null,
              ...
              <div key={i} style={{ padding: '14px 20px', flex: '1 1 auto', borderRight: i < arr.length - 1 ? `1px solid ${c.border}` : 'none', minWidth: 100 }}>
          ...
          {r.ingredients && (
            <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 18 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, paddingBottom: 10, borderBottom: `2px solid ${c.accentLight}` }}>
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 21, fontWeight: 700, color: c.accent }}>Ingredienti</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: c.muted, letterSpacing: '.08em', textTransform: 'uppercase' }}>Porzioni</span>
                  <div style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${c.border}`, borderRadius: 8, overflow: 'hidden' }}>
                    <button onClick={() => setCurrentServings(s => Math.max(1, s - 1))}
                      style={{ background: c.bg, border: 'none', width: 32, height: 32, cursor: 'pointer', fontSize: 18, color: c.muted, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                    <div style={{ width: 36, textAlign: 'center', fontWeight: 700, fontSize: 15, color: c.text, borderLeft: `1px solid ${c.border}`, borderRight: `1px solid ${c.border}`, lineHeight: '32px' }}>
                      {currentServings}
                    </div>
                    <button onClick={() => setCurrentServings(s => s + 1)}
                      style={{ background: c.bg, border: 'none', width: 32, height: 32, cursor: 'pointer', fontSize: 18, color: c.accent, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                  </div>
                  {currentServings !== baseServings && (
                    <button onClick={() => setCurrentServings(baseServings)}
                      style={{ background: 'none', border: 'none', color: c.muted, fontSize: 11, cursor: 'pointer', textDecoration: 'underline' }}>reset</button>
                  )}
                </div>
              </div>
              <div style={{ whiteSpace: 'pre-wrap', lineHeight: 2.1, fontSize: 15, fontWeight: 600 }}>
            ...
            <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 18 }}>
              ...
              {parsedSteps.map((step, i) => step.trim() && (
                <div key={i} style={{ display: 'flex', gap: 14, marginBottom: 18 }}>
                  <div style={{ minWidth: 30, height: 30, borderRadius: '50%', background: c.accentLight, color: c.accent, fontWeight: 700, fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2, fontFamily: "'Cormorant Garamond',serif" }}>{i + 1}</div>
                  <div style={{ whiteSpace: 'pre-wrap', lineHeight: 1.85, fontSize: 15, paddingTop: 4 }}>{step}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {r.notes && (
            <div style={{ background: '#FFFBF0', border: `1px solid #EDD080`, borderLeft: `4px solid ${c.accentMid}`, borderRadius: '0 12px 12px 0', padding: '18px 22px' }}>
    ...
    return (
      <div style={A.wrap}>
        ...
        <div style={A.header}>
          ...
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, color: c.accent, fontWeight: 600 }}>
        ...
        <div style={{ maxWidth: 780, margin: '0 auto', padding: '26px 24px 70px' }}>
          {error && <div style={A.err}>{error}</div>}
          ...
          <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 16 }}>
            ...
            <div style={A.field}>
              ...
              <input style={{ ...A.input, fontSize: 16, fontWeight: 600 }} placeholder="Es. Tiramisù classico"
            ...
            <div className="g2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={A.field}>
                ...
                <AutocompleteInput value={form.type} onChange={v => sf('type', v)} suggestions={types} placeholder="Es. Dolce, Salato, Pasta..." style={A.input} field="type" />
              </div>
              <div style={A.field}>
                <label style={A.label}>Peso (facoltativo)</label>
                <input style={A.input} placeholder="Es. 750g" value={form.weight} onChange={(e: ChangeEvent<HTMLInputElement>) => sf('weight', e.target.value)} />
              </div>
            </div>
            <div className="g2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={A.field}>
                ...
                <div style={{ color: c.muted, fontSize: 11, marginTop: 5 }}>Numero di porzioni su cui sono calcolati gli ingredienti</div>
              </div>
              <div style={A.field}>
          ...
          <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 16 }}>
            ...
            {photoPreview ? (
              <div>
                <img src={photoPreview} alt="cover" style={{ width: '100%', maxHeight: 270, objectFit: 'cover', borderRadius: 10, display: 'block', boxShadow: `0 4px 16px ${c.shadow}` }} />
                <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                  <button className="hbtn" style={A.btnO} onClick={() => fileRef.current?.click()} disabled={uploading}>
              ...
              <div style={{ border: `2px dashed ${c.border}`, borderRadius: 10, padding: '36px 24px', textAlign: 'center', cursor: uploading ? 'default' : 'pointer', background: c.bg }}
                onClick={() => !uploading && fileRef.current?.click()}>
                <div style={{ fontSize: 38, marginBottom: 10 }}>📷</div>
                <div style={{ color: c.muted, fontSize: 14, marginBottom: 4, fontWeight: 600 }}>
                  {uploading ? '⏳ Compressione e caricamento...' : 'Clicca per aggiungere una foto'}
                </div>
          ...
          <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 16 }}>
            ...
            <div style={{ color: c.muted, fontSize: 12, marginBottom: 12 }}>
              Inserisci le quantità per {form.servings || 1} porzione{(form.servings || 1) > 1 ? 'i' : ''}
            </div>
            <textarea style={{ ...A.input, minHeight: 175, lineHeight: 2.1, fontWeight: 600 }}
              placeholder={"- 500g farina 00\n- 200g zucchero\n- 4 uova\n- 250ml latte intero\n..."}
          ...
          <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 16 }}>
            ...
            <div style={{ color: c.muted, fontSize: 12, marginBottom: 14 }}>
              <strong>Invio</strong> = nuovo passo numerato &nbsp;·&nbsp; <strong>Shift+Invio</strong> = a capo nello stesso passo
            </div>
          ...
          <div style={{ ...A.cardBox, padding: '22px 26px', marginBottom: 30 }}>
            ...
            <textarea style={{ ...A.input, minHeight: 100, lineHeight: 1.9, fontStyle: 'italic' }}
